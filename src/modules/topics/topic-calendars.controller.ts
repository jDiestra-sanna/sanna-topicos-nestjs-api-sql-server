...
@Controller('topic-calendars')
export class TopicCalendarsController {
  constructor(private readonly topicsService: TopicsService) {}

  @Get()
  async findAllCalendars(@Query() query: ReqQuery, @AuthUser() authUser: User, @Res() res: Response) {
    ...
    const data = await this.topicsService.findAllCalendars(query, authUser.id, authUser.is_central);

    if (!data) return rsp404(res, 'No se encontró ningún campus asignado para este usuario');
